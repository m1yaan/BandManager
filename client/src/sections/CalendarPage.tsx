...
import { ChevronLeft, ChevronRight, MapPin, Music2, Ticket, CheckCircle2 } from 'lucide-react';

const MONTHS_RU = ['Январь','Февраль','Март','Апрель','Май','Июнь','Июль','Август','Сентябрь','Октябрь','Ноябрь','Декабрь'];
const WEEKDAYS = ['Пн','Вт','Ср','Чт','Пт','Сб','Вс'];

const TYPE_ICONS: Record<string, React.ReactNode> = {
  concert: <Ticket className="w-3 h-3" />,
  stop:    <Music2 className="w-3 h-3" />,
  ...
  return (
    <div className="p-3 rounded-xl" style={{ background: event.color + '15', border: `1px solid ${event.color}40` }}>
      <div className="flex items-center gap-2 mb-1">
        <span style={{ color: event.color }}>{TYPE_ICONS[event.type] ?? TYPE_ICONS.stop}</span>
      ...
      {event.type === 'offer' && event.city && (
        <p className="text-[12px] mt-0.5 flex items-center gap-1" style={{ color: 'var(--text-secondary)' }}>
          <MapPin className="w-3 h-3" />{event.city}
        </p>
      )}
      {(event.band_name || event.singer_name) && (
...
export default function CalendarPage() {
  ...
  const eventsForDay = useCallback((day: number): CalendarEvent[] => {
  ...
  const monthEventCount = useMemo(() => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      eventsForDay(day).forEach(ev => seen.add(ev.id));
    }
    return seen.size;
  }, [eventsForDay, daysInMonth]);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-wrap items-center gap-4 text-[12.5px]" style={{ color: 'var(--text-secondary)' }}>
        <div className="flex items-center gap-1.5">
          <div className="w-2.5 h-2.5 rounded-full" style={{ background: '#a78bfa' }} />Концерт
        </div>
        <div className="flex items-center gap-1.5">
          <div className="w-2.5 h-2.5 rounded-full" style={{ background: '#6366f1' }} />Остановка тура
        </div>
        <div className="flex items-center gap-1.5">
                  ...
                  >
                    ...
                    <div className="space-y-0.5">
                      {dayEvents.slice(0, 2).map(ev => (
                        ...
                        >
                          {TYPE_ICONS[ev.type] ?? TYPE_ICONS.stop}
                          <span className="truncate">
                            {ev.type === 'stop' ? `${ev.title}: ${ev.city}` : ev.title}
                          </span>
                        </div>
                      ))}
                      {dayEvents.length > 2 && (

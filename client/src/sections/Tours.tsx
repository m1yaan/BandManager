...
import { toursApi, bandsApi, songsApi, Tour, Band, Song, TourStop, RiderItem } from '../lib/api';
...
import {
  ...
  Ticket, Settings, CheckCircle2, AlertCircle, Clock, X, DollarSign,
} from 'lucide-react';
...
// ── Компонент вкладок в раскрытом туре ────────────────────────────────────────
type TourTab = 'setlist' | 'stops' | 'rider' | 'finances';

function TourExpandedPanel({ tour, isOwner }: { tour: TourWithDetails; isOwner: boolean; onUpdate?: () => void }) {
  const [tab, setTab] = useState<TourTab>('setlist');
  const [stops, setStops] = useState<TourStop[]>(tour.stops ?? []);
  const [rider, setRider] = useState<RiderItem[]>(tour.rider ?? []);
  const [finances, setFinances] = useState<Partial<Tour>>({});
  const [newStop, setNewStop] = useState({ city: '', event_date: '', avg_ticket_price: '' });
  ...
  useEffect(() => {
    ...
    if (tab === 'finances') {
      toursApi.getFinances(tour.id).then(f => {
        ...
        setFinForm({
          fee: f.fee?.toString() ?? '',
          tax_percent: f.tax_percent?.toString() ?? '',
          agent_commission_percent: f.agent_commission_percent?.toString() ?? '',
          transport: f.transport?.toString() ?? '',
          per_diem: f.per_diem?.toString() ?? '',
          hotel: f.hotel?.toString() ?? '',
          other_expenses: f.other_expenses?.toString() ?? '',
        });
      }).catch(console.error);
    }
  ...
  const addStop = async () => {
    ...
    try {
      const stop = await toursApi.addStop(tour.id, {
        ...
        avg_ticket_price: newStop.avg_ticket_price ? parseFloat(newStop.avg_ticket_price) : 0,
      });
      setStops(prev => [...prev, stop]);
      setNewStop({ city: '', event_date: '', avg_ticket_price: '' });
  ...
  const saveFinances = async () => {
    try {
      ...
      await toursApi.getFinances(tour.id).then(setFinances);
  ...
  // Расчёт прибыли
  const fee = parseFloat(finForm.fee) || finances.fee || 0;
  const tax = parseFloat(finForm.tax_percent) || finances.tax_percent || 0;
  const agentPct = parseFloat(finForm.agent_commission_percent) || finances.agent_commission_percent || 0;
  const transport = parseFloat(finForm.transport) || finances.transport || 0;
  const perDiem = parseFloat(finForm.per_diem) || finances.per_diem || 0;
  const hotel = parseFloat(finForm.hotel) || finances.hotel || 0;
  const other = parseFloat(finForm.other_expenses) || finances.other_expenses || 0;
  const netFee = fee * (1 - tax / 100) * (1 - agentPct / 100);
  const totalExpenses = transport + perDiem + hotel + other;
  const profit = netFee - totalExpenses;

  const TABS: { id: TourTab; label: string }[] = [
    { id: 'setlist',  label: 'Сет-лист' },
    { id: 'stops',    label: 'Города' },
    { id: 'rider',    label: 'Райдер' },
        ...
        {tab === 'setlist' && (
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-[12px] font-semibold uppercase tracking-wide" style={{ color: 'var(--text-tertiary)' }}>Программа выступления</span>
            </div>
          ...
          <div className="space-y-3">
            {stops.length > 0 ? (
              <div className="space-y-2">
                {stops.map(stop => (
                  <div key={stop.id} className="flex items-center gap-3 p-3 rounded-xl" style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-base)' }}>
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        {stop.avg_ticket_price > 0 && (
                          <span className="text-[12px]" style={{ color: 'var(--success)' }}>{stop.avg_ticket_price.toLocaleString()} ₽</span>
                        )}
                      </div>
                    </div>
                    {isOwner && (
                      <button onClick={() => deleteStop(stop.id)} className="btn btn-ghost btn-icon" style={{ color: 'var(--text-tertiary)' }}>
            ...
            {isOwner && (
              <div className="flex gap-2 mt-3">
                <input className="input-base flex-1" style={{ padding: '8px 12px', fontSize: 13 }} value={newStop.city} onChange={e => setNewStop(p => ({ ...p, city: e.target.value }))} placeholder="Город" />
                <input type="date" className="input-base" style={{ padding: '8px 12px', fontSize: 13, width: 140 }} value={newStop.event_date} onChange={e => setNewStop(p => ({ ...p, event_date: e.target.value }))} />
                <input type="number" className="input-base" style={{ padding: '8px 12px', fontSize: 13, width: 120 }} value={newStop.avg_ticket_price} onChange={e => setNewStop(p => ({ ...p, avg_ticket_price: e.target.value }))} placeholder="Цена ₽" />
          ...
          <div className="space-y-3">
            {rider.length > 0 && (
              <div>
                ...
                {rider.map(item => (
                  <div key={item.id} className="flex items-center gap-3 py-2.5 border-b" style={{ borderColor: 'var(--border-subtle)' }}>
                    <div className="flex-1">
                      <p className="text-[13.5px]" style={{ color: 'var(--text-primary)', textDecoration: item.status === 'confirmed' ? 'line-through' : 'none', opacity: item.status === 'confirmed' ? 0.6 : 1 }}>
                        {item.item_name}
                      </p>
                    ...
                    {isOwner && (
                      <div className="flex items-center gap-1">
                        {(['pending', 'confirmed', 'problem'] as const).map(s => (
                          <button
                            key={s}
                            onClick={() => updateRiderStatus(item, s)}
                        ...
                        <button onClick={() => deleteRiderItem(item.id)} className="btn btn-ghost btn-icon ml-1" style={{ color: 'var(--text-tertiary)' }}>
                          <X className="w-3.5 h-3.5" />
                        </button>
                      </div>
                    )}
            ...
            {isOwner && (
              <div className="flex gap-2 mt-2">
                <input className="input-base flex-1" style={{ padding: '8px 12px', fontSize: 13 }} value={newRiderItem} onChange={e => setNewRiderItem(e.target.value)} onKeyDown={e => e.key === 'Enter' && addRiderItem()} placeholder="Новый пункт райдера…" />
        ...
        {tab === 'finances' && (
          <div>
            ...
            {editingFinances ? (
              ...
              <div className="space-y-2">
                {[
                  { label: 'Гонорар', value: finances.fee || 0, isIncome: true },
                  { label: `Налог (${finances.tax_percent || 0}%)`, value: -(finances.fee || 0) * (finances.tax_percent || 0) / 100 },
                  { label: `Комиссия агента (${finances.agent_commission_percent || 0}%)`, value: -(finances.fee || 0) * (finances.agent_commission_percent || 0) / 100 },
                  { label: 'Транспорт', value: -(finances.transport || 0) },
                  { label: 'Суточные', value: -(finances.per_diem || 0) },
                  { label: 'Гостиница', value: -(finances.hotel || 0) },
                  { label: 'Прочие расходы', value: -(finances.other_expenses || 0) },
                  ...
                  <div key={label} className="flex items-center justify-between py-1.5" style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                    ...
                    <span className="text-[13px] font-medium" style={{ color: isIncome ? 'var(--success)' : value < 0 ? 'var(--error)' : 'var(--text-primary)' }}>
  ...
  return (
    <div className="space-y-6 animate-fade-in">
          ...
          {tours.map(tour => (
            <div key={tour.id} className="card overflow-hidden">
              <div className="flex items-start gap-4 px-5 py-4">
                <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5" style={{ background: 'rgba(251,146,60,0.1)', color: '#fb923c' }}>
                  <MapPin className="w-4 h-4" />
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="text-[14.5px] font-semibold" style={{ color: 'var(--text-primary)' }}>{tour.program_name}</h3>
                  ...
                  {tour.created_by === user?.id && (
                    <>
                      ...
                      <Tooltip label="Удалить">
                        <button onClick={() => setDeleteTarget(tour)} className="btn btn-ghost btn-icon" style={{ color: 'var(--text-tertiary)' }}>
      ...
      {showForm && (
        ...
        >
          <div className="space-y-5">
            <div>
              <label className="block text-[13px] font-medium mb-2" style={{ color: 'var(--text-secondary)' }}>Название программы *</label>
              <input autoFocus className="input-base" value={form.program_name} onChange={e => setForm({ ...form, program_name: e.target.value })} placeholder="Мировой тур 2025" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-[13px] font-medium mb-2" style={{ color: 'var(--text-secondary)' }}>Город</label>
                <input className="input-base" value={form.city} onChange={e => setForm({ ...form, city: e.target.value })} placeholder="Москва" />
              </div>
              <div>
            ...
            <div>
              ...
              <select className="input-base" value={form.band_id} onChange={e => setForm({ ...form, band_id: e.target.value })}>
            ...
            {songs.length > 0 && (
              <div>
                ...
                <div className="max-h-36 overflow-y-auto rounded-xl p-2 space-y-0.5" style={{ background: 'var(--bg-elevated)', border: '1px solid var(--border-base)' }}>
                  {songs.map(s => (
                    <label key={s.id} className="flex items-center gap-3 px-3 py-2 rounded-lg cursor-pointer" style={{ background: selectedSongs.includes(s.id) ? 'rgba(96,165,250,0.08)' : 'transparent' }}>
                      <input type="checkbox" checked={selectedSongs.includes(s.id)} onChange={() => setSelectedSongs(prev => prev.includes(s.id) ? prev.filter(x => x !== s.id) : [...prev, s.id])} className="w-4 h-4 accent-indigo-500" />
      ...
      {deleteTarget && (
        <ConfirmDialog
          title="Удалить тур?"
          description={`Вы уверены, что хотите удалить тур «${deleteTarget.program_name}»? Все данные тура будут удалены.`}

...
export default function Layout({ activeSection, onNavigate, children }: Props) {
  ...
  const navItems = ALL_NAV_ITEMS.filter(item => !item.managerOnly || isManager);
  ...
  const Sidebar = () => (
    <aside
      className={`fixed inset-y-0 left-0 z-40 w-[220px] flex flex-col transition-transform duration-300 ease-in-out ${mobileOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0`}
      style={{ background: 'var(--bg-surface)', borderRight: '1px solid var(--border-base)' }}
    >
      <div className="flex items-center gap-3 px-5 h-[60px] flex-shrink-0" style={{ borderBottom: '1px solid var(--border-subtle)' }}>
        <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: 'var(--accent)', boxShadow: '0 0 12px rgba(99,102,241,0.4)' }}>
      ...
      <div className="px-5 py-2" style={{ borderBottom: '1px solid var(--border-subtle)' }}>
        <span
          className="badge text-[11px]"
          style={{
            background: isManager ? 'var(--accent-muted)' : 'rgba(52,211,153,0.1)',
            color: isManager ? 'var(--accent)' : '#34d399',
          }}
        >
          {isManager ? 'Менеджер' : 'Артист'}
        </span>
      </div>
      ...
      <div className="px-3 pb-4 pt-3 space-y-1 flex-shrink-0" style={{ borderTop: '1px solid var(--border-subtle)' }}>
        <button onClick={toggleTheme} className="nav-btn w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-[13.5px] font-medium transition-all" style={{ color: 'var(--text-secondary)' }}>
          {theme === 'dark' ? <Sun className="w-4 h-4" style={{ color: 'var(--warning)' }} /> : <Moon className="w-4 h-4" style={{ color: 'var(--info)' }} />}
        ...
        >
          {user?.avatar_url ? (
            ...
            <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 text-[11px] font-bold text-white" style={{ background: 'var(--accent)' }}>
              {initials}
            </div>
          )}
        ...
        <button onClick={signOut} className="nav-btn w-full flex items-center gap-3 px-3 py-2 rounded-xl text-[13px] transition-all" style={{ color: 'var(--text-tertiary)' }}>
  ...
  return (
    <div className="min-h-screen flex" style={{ background: 'var(--bg-base)' }}>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-30 lg:hidden" style={{ background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)' }} onClick={() => setMobileOpen(false)} />
      )}
